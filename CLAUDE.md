...
- Local DB: SQLite via Drift (`drift ^2.18.0`), db at `assets/db/sanatan_guide.db`
...
## Conventions
- Business logic lives in domain layer. UI widgets stateless when possible.
- `const` constructors for all static widgets.
- No `setState` for app-wide state — use Riverpod providers.
- All async calls must handle loading/error states.
- Feature-based folder layout: each feature has presentation/domain/data.

## Quality gates (enforced by hooks)
- `flutter analyze` runs before Edit/Write (PreToolUse hook).
- New features include a widget test.

## Skills loaded by default
- frontend-design
- simplify
- pdf, docx, xlsx (docs + spreadsheets)
- skill-creator, webapp-testing
- superpowers (community)

## MCP servers
- dart (official Dart/Flutter tooling)
- github (repo, issues, PRs)
- sqlite (scoped to `assets/db/sanatan_guide.db`)

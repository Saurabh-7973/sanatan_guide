...
Actual layout in this repo (clean architecture):

```
lib/
  presentation/
    theme/
      app_colors.dart          (legacy)
      app_theme.dart           (legacy ThemeData)
      design_tokens.dart       (NEW — DColors/LColors/Fonts/Spacing/Radii/Motion/Glows)
      design_typography.dart   (NEW — AppText.* styles)
    shared/
      widgets/
        heritage_widgets.dart  (NEW — BindingLine, LeafThread, DandaCoord, AIThinkingDots, SanskritText)
        warm_backdrop.dart     (legacy)
        sacred_ornaments.dart  (legacy painters)
    features/
      home/
      scripture_reader/   (verse-detail, chapter-list, verse-list)
      bookmarks/
      festivals/
      onboarding/
      learning_path/      (= practice)
      search/
      settings/
      // to add: library, ai_chat
  domain/  data/  core/
```

Use `package:sanatan_guide/...` imports — project lints require package imports for files in `lib/`.

Build heritage widgets first. Every screen uses them.

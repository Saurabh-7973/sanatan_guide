        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: widget.isDark
                ? [BoxShadow(color: color.withOpacity(glowOpacity), blurRadius: 6)]
                : null,
          ),
        );
      },
...
class DandaCoord extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final coordText = parts.map(toDevanagari).join('·');
...
/// Single AI-thinking vocabulary across the app.
...
class _AIThinkingDotsState extends State<AIThinkingDots>
  ...
  double _opacityFor(double t, double phase) {
    ...
    // 0.0 → 0.2: ramp up to peak. 0.2 → 0.4: ramp down. Rest of cycle: base.
    if (shifted < 0.2) {

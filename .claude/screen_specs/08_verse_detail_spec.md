...
4. **Scrolling content** (`screen-content`, fills remaining height above util bar):
   ...
   - **Translation section** (`margin: 32px 24px 0`):
     ...
     - Body: `Fonts.serif` (regular, **not** italic) 16 px `line-height 1.7` **`text-align: left`** `text-1`, `padding: 0 8px`. Flowing prose — Geeta Press / Chinmaya convention, reads like a book, not a quote card. Source: `verse.translation ?? verse.english`. Wrap any quoted dialogue in standard double-quotes if the source uses them.
     - **Attribution** (`margin-top: 12px`, `Fonts.sans` 10 px w500 `letter-spacing 0.18em uppercase` `text-align: right text-3`, `padding-right: 8px`): `— {translator}`. Ex: `— Swami Sivananda`. Omit the row if no translator metadata.

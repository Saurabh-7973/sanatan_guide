...
const _kCrashlyticsEnabledKey = 'privacy_crashlytics_enabled';
...
class CrashlyticsEnabled extends _$CrashlyticsEnabled {
  ...
  Future<void> _load() async {
    ...
    final v = prefs.getBool(_kCrashlyticsEnabledKey);
    if (v != null) {
      state = v;
      await _apply(v);
    }
  ...
  Future<void> setEnabled(bool enabled) async {
    state = enabled;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_kCrashlyticsEnabledKey, enabled);
